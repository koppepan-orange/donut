...
HomD.appendChild(renderer.domElement);//レンダラーをcanvasの代わり
...
let backGrounds = [];
for(let ob of Objects.filter(ob => ob.start)){
    placeObject(ob.startF);
}
...
function updateCameraMovement() {
    ...
    if (OBS.keys.w || OBS.keys.s || OBS.keys.a || OBS.keys.d) {
        yawBase += moveX;
        moveX = 0;
    }
